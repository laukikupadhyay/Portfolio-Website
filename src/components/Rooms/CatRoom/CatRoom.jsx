...
function CatRoom() {
  ...
  const locationNotUpdated =
    !userInfo.location.coordinates[1] && !userInfo.location.coordinates[0];
  ...
  const getCategoryRooms = async (category) => {
    ...
    try {
      const res = await fetch(
        process.env.REACT_APP_BACKEND_URL + `groups/` + category,
        {
          method: "GET",
        }
      );
          ...
          <>
            {locationNotUpdated ? (
              <div className={styles.room}>
                <i>
                  <h3>
                    Location not updated, Go to edit profile to update the
                    location ....
                  </h3>
                </i>
              </div>
              ...
              <>
                {catRooms.slice(0, visibleRooms).map((eachRoom) => (
                  <div key={eachRoom.id}>
                    <EachCatRoom eachRoom={eachRoom} value={value} />
                  </div>
                ))}
                {visibleRooms < catRooms.length && (
